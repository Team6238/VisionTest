...
void TestPipeline::Process(cv::Mat& source0){
	...
	double hsvThresholdHue[] = {0.0, 70.0};
	double hsvThresholdSaturation[] = {26.26618413444903, 100.80545245095735};
	double hsvThresholdValue[] = {140.3129562010868, 255.0};
 ...
 */
cv::Mat* TestPipeline::GetCvResizeOutput(){
 ...
 */
cv::Mat* TestPipeline::GetCvErodeOutput(){
 ...
 */
cv::Mat* TestPipeline::GetMaskOutput(){
 ...
 */
std::vector<cv::KeyPoint>* TestPipeline::GetFindBlobsOutput(){
	 ...
	 */
	void TestPipeline::cvResize(cv::Mat &src, cv::Size &dSize, double fx, double fy, int interpolation, cv::Mat &dst) {
	 ...
	 * @param dst Output Image.
	 */
	void TestPipeline::cvErode(cv::Mat &src, cv::Mat &kernel, cv::Point &anchor, double iterations, int borderType, cv::Scalar &borderValue, cv::Mat &dst) {
		 ...
		 * @param output The image in which to store the output.
		 */
		void TestPipeline::mask(cv::Mat &input, cv::Mat &mask, cv::Mat &output) {
	...
	void TestPipeline::findBlobs(cv::Mat &input, double minArea, double circularity[], bool darkBlobs, std::vector<cv::KeyPoint> &blobList) {
		blobList.clear();
